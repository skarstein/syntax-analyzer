...
static void declarator(int typespec)
{
	...
	match('[');
        ...
        unsigned len=expectNum();
	//match(NUM);
	match(']');
        Type t(ARRAY,typespec,ind);
        t._length=len;
        variableDecl(name,t);
    }
    else
        variableDecl(name,Type(SCALAR,typespec,ind));
}
...
static void globalDeclarator(int spec)
{
    ...
    if (lookahead == '[') {
	match('[');
        int length = expectNum();
	match(']');
        Type t(spec,ind,length);
        variableDecl(name,t);
	...
	match(')');
        ...
        variableDecl(name,Type(SCALAR,spec,ind));
}
